...
constructor(
  ...
  private getProjectWithTasks(projectId :  number) : Observable<Task[]> {
    return this.apiService.get<Task[]>(`${enviroment.apiProjectUrl}/${projectId}/tasks`, {withCredentials: true});
  }
  ...
  getProjectId(projectKey : string) : Observable<number> {
    return this.apiService.get<number>(`${enviroment.apiProjectUrl}/${projectKey}`, {withCredentials: true})
  }
}
